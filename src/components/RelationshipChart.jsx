...
function CharacterPortrait({ character, crossOrigin }) {
    ...
    return character.photo && character.photo !== failedUrl
        ? <img className="relationship-chart-portrait" src={character.photo} alt="" crossOrigin={crossOrigin} onError={() => setFailedUrl(character.photo)} />
...
function PopoverShell({ label, onClose, children }) {
    return <aside className="relationship-chart-popover relationship-chart-side-panel" aria-label={label}>
        ...
        <div className="relationship-chart-side-save">
            <button type="submit" className="form-primary-submit">Save relationship chart</button>
    ...
    return <PopoverShell label={`Edit relationship between ${connection.source.name} and ${connection.target.name}`} onClose={onClose}>
        ...
        {connection.episode !== episode
            ? <p className="eventform-field-note">State: {relationshipChartEpisodeLabel(data, connection.episode)}
                {episodes.includes(episode) && <button type="button" onClick={forkHere}> New state at {relationshipChartEpisodeLabel(data, episode)}</button>}</p>
            : null}
...
function CharacterEditPopover({ data, onChange, character, authors, authorsLoading, episode, episodes, onStartLink, onRemove, onClose }) {
    ...
    return <PopoverShell label={`Edit ${character.name || "character"}`} onClose={onClose}>
        ...
        <div className="relationship-chart-editor-actions">
            ...
            <button type="button" className="relationship-chart-form-remove" onClick={onRemove}>Remove character</button>
        </div>
    </PopoverShell>;
}
...
function GroupEditPopover({ data, onChange, group, onRemove, onClose }) {
    ...
    return <PopoverShell label={`Edit ${group.label || "group"}`} onClose={onClose}>
        ...
        <div className="relationship-chart-editor-actions">
            <button type="button" className="relationship-chart-form-remove" onClick={onRemove}>Remove group</button>
        </div>
    </PopoverShell>;
}

export default function RelationshipChart({ data, onChange, projectTitle, episodeCount = 0, editable = false, isAdmin = false, headerControls = null, onEpisodePublicChange = null, busy = false, exportView = false, forcedEpisode = null, measureNonce = 0, useLoafFrame = false }) {
    ...
    const activeDirection = selected?.kind === "relationship" && active ? relationshipChartDirection(active) : null;
    ...
    async function downloadImage() {
        ...
        const canvasNode = chart?.querySelector(".relationship-chart-canvas");
        if (chart && canvasNode) {
            // Measure only after fonts settle, and fit long titles without wrapping.
            await document.fonts.ready;
            const title = chart.querySelector(".relationship-chart-heading h2");
            ...
            const exportCards = [...canvasNode.querySelectorAll(".relationship-chart-person")];
            ...
            const edges = [...chart.querySelectorAll(".relationship-chart-person")].map((card, index) => {
    ...
    return <section ref={sectionRef} className={`relationship-chart${useLoafFrame ? " frame-loaf" : ""}${editable && (editingCharacter || editingConnection || editingGroup) ? " has-side-editor" : ""}${expanded ? " is-expanded" : ""}${exportView ? " is-exporting" : ""}`} aria-labelledby="relationship-chart-heading">
        ...
        <div className="relationship-chart-canvas" ref={canvasRef} style={{ "--canvas-height": `${canvasHeight}px` }}>
            {(data.groups || []).map((group) => {
                ...
                return <div key={group.id} className={`relationship-chart-group ${group.shape}`} style={{ left: `${bounds.left}%`, top: `${bounds.top}%`, width: `${bounds.width}%`, height: `${bounds.height}%`, '--group-color': group.color, borderStyle: group.line_style || 'solid' }} role="group" aria-label={`${group.label}: ${characters.filter((character) => group.character_ids.includes(character.id)).map((character) => character.name).join(', ')}`}>
                    ...
                    {editable && ["top", "bottom", "left", "right"].map((edge) => <button key={edge} type="button" className={`relationship-chart-group-edge ${edge}`} aria-label={`Adjust ${group.label || "group"} ${edge} margin`} title={`Drag to adjust ${edge} margin`}
        ...
        <dialog ref={dialog} className="relationship-chart-dialog" aria-labelledby="relationship-chart-detail-title" onClick={(event) => { if (event.target === dialog.current) dialog.current.close(); }}>
            ...
            {active && <>
                ...
                <h3 id="relationship-chart-detail-title">{active.name || active.label}</h3>
                {active.thai_name && <p lang="th" className="relationship-chart-thai-name">{active.thai_name}</p>}
                <p className="relationship-chart-detail-subtitle">{selected.kind === "character" ? active.role
